...
import com.swalif.sa.repository.searchRepository.SearchInfo
...
class SearchManagerFakeData @Inject constructor(
...
) : SearchManager<SearchInfo> {
    private var job: Job? = null
    private val scope = CoroutineScope(dispatcherProvider.default)
    override var onSearchEventListener: SearchEvent<SearchInfo>? = null
    ...
    private val fakeData = listOf<UserInfo>(
        ...
        UserInfo(
            "Ahmed",
            "",
            "https://preview.redd.it/tried-to-make-an-anime-like-version-of-my-avatar-using-a-i-v0-2dmjoo8g1g5a1.jpg?auto=webp&s=befb3b3269e597e28921db699d7138f02e8ff8a1",
            Gender.Male
        ),
        UserInfo(
            "salma",
    ...
    override fun registerSearchEvent() {
        job?.cancel()
        job = scope.launch {
            while (true) {
                delay(4000)
                if (isActive) {
                    val userInfo = fakeData.random()
                    onSearchEventListener?.onEvent(SearchInfo(userInfo,SearchStateResult.FOUND_USER))
    ...
    override fun updateSearchState(searchStateResult: SearchStateEvent) {
        when(searchStateResult){
            SearchStateEvent.IGNORE -> {
                currentUserInfo?.let {
                    onSearchEventListener?.onEvent(it)
                }
            }
            SearchStateEvent.ACCEPT -> TODO()
        }
    }
